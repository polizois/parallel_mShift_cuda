...
*/
/*
    This iteration of mean shift does not use the GPU's SHARED MEMORY and in order to
		calculate a sum of N doubles, it performs atomic increases on a certain memory location.
		So every thread has to wait it's turn in order to perform the increase and the sumation becomes
		serial.(actually worse than serial because of the warp bound threads).
*/
...
//Using this so only one thread at a time can change the value in a (global) memory address
//avoiding race conditions
#if __CUDA_ARCH__ < 600
__device__ double aAdd(double* address, double val)
{
...
int main(int argc, char *argv[])
{
  ...
  // Loading data from file to table
	...
	// Copy pointd from host memory to device memory
	...
	do
	{
		...
		yNext<<<numBlocks, threadsPerBlock>>>(dx, dy, dtemp, ROWS, COLS, blockNum(ROWS, THREADSPERBLOCK));
		cudaThreadSynchronize();

    con=1;
		cudaMemcpy(temp[0], dtemp, tempRows*(COLS+1)*sizeof(double), cudaMemcpyDeviceToHost);

		//Calculating every new y and checking if the corresponding mean shift converges
...
__global__ void yNext(double *x, double *y, double *out, int rows, int cols, int blocksForY)
{
  ...
  int bid = blockIdx.y*gridDim.x + blockIdx.x;
	...
	// Clear the place in memory that is about to be written
	if(bid < tempRows) if(tid1d == 0) out[cols*tempRows+bid]=0;
	__syncthreads();

	// Every thread in a block(if in limits) increases the value of the accornding place of out
	// Only one thread at a time, can perform the increase.
	if(bid < tempRows) aAdd(&out[cols*tempRows+bid], inRows * inVar * gaus);

	// The exact same thing done here for every dimention(colum)
	for(i=0;i<cols;i++)
	{
		if(bid < tempRows) if(tid1d == 0) out[i*tempRows+bid]=0;
		__syncthreads();

		if(bid < tempRows) aAdd(&out[i*tempRows+bid], inRows * inVar * gaus * x[i*rows+id]);
	}
...
int blockNum(int N, int thPerBlock)
{
	...
	if(num*thPerBlock - N < 32 && num*thPerBlock - N != 0) num+=1;
...
void exportResults(double **x, int rows, int cols, int threads, double dur, int iters)
{
	...
	sprintf(name, "./results/non_(%d_%d)_(%d_%.3f_%d).bin", rows, cols, threads, dur, iters);
